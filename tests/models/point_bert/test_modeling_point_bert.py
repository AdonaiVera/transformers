...
""" Testing suite for the PyTorch Point-BERT model. """


import inspect
import unittest

from transformers import PointBERTConfig, is_torch_available, is_vision_available
...
if is_torch_available():
    ...
    from transformers.models.point_bert.modeling_point_bert import POINT_BERT_PRETRAINED_MODEL_ARCHIVE_LIST


if is_vision_available():
    from PIL import Image

    from transformers import PointBERTFeatureExtractor


class PointBERTConfigTester(ConfigTester):
...
class PointBERTModelTester:
    ...
    def create_and_check_model(self, config, pixel_values, labels):
        model = PointBERTModel(config=config)
    ...
    def create_and_check_for_binary_image_segmentation(self, config, pixel_values, labels):
        config.num_labels = 1
        model = PointBERTForSemanticSegmentation(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class PointBERTModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            PointBERTModel,
            PointBERTForSemanticSegmentation,
    ...
    def setUp(self):
        self.model_tester = PointBERTModelTester(self)
        self.config_tester = PointBERTConfigTester(self, config_class=PointBERTConfig)
    ...
    @unittest.skip("Point-BERT does not use inputs_embeds")
    def test_inputs_embeds(self):
        pass

    @unittest.skip("Point-BERT does not have get_input_embeddings method and get_output_embeddings methods")
    ...
    def test_model_from_pretrained(self):
        for model_name in POINT_BERT_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = PointBERTModel.from_pretrained(model_name)
...
@require_torch
class PointBERTModelIntegrationTest(unittest.TestCase):
    @slow
    def test_inference_image_segmentation_ade(self):
        ...
        model = PointBERTForSemanticSegmentation.from_pretrained("nvidia/point_bert-b0-finetuned-ade-512-512").to(
            torch_device
        )
    ...
    def test_inference_image_segmentation_city(self):
        ...
        model = PointBERTForSemanticSegmentation.from_pretrained(
            "nvidia/point_bert-b1-finetuned-cityscapes-1024-1024"
        ).to(torch_device)
